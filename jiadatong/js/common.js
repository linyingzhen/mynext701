...
function addToCart(goodsId, parentId, isnowbuy, extCode) {
	...
	var number = 1;
	var formBuy = document.forms['unc_FORMBUY'];
	var quick = 0;
...
/* 组合购买__添加商品到购物车__Start By www.m775.com */
...
/* 组合购买__添加商品到购物车__End By www.m775.com */
...
function addToCartResponse(result) {
	if (result.error > 0) {
		...
		var cartInfo = document.getElementById('unc_CARTINFO');
...
function gotoPageResponse_question(result) {
	document.getElementById("unc_QUESTION").innerHTML = result.content;
}
...
/* 代码增加_end By www.m775.com */

/* 代码修改_start 整个替换掉即可 By www.m775.com */
/*******************************************************************************
 * 评论的翻页函数
 */
...
/* 代码修改_end By www.m775.com */

function gotoPageResponse(result) {
	document.getElementById("unc_COMMENT").innerHTML = result.content;
}
...
function gotoBuyPageResponse(result) {
	document.getElementById("unc_BOUGHT").innerHTML = result.result;
}
...
function bidResponse(result) {
	if (result.error == 0) {
		document.getElementById('unc_SNATCH').innerHTML = result.content;
...
function newPriceResponse(result) {
	document.getElementById('unc_PRICE_LIST').innerHTML = result;
}
...
function set_insure_status() {
	...
	if (shippingId > 0) {
		...
		// 设置配送保价，为0隐藏
		if (document.getElementById("unc_insure_cell")) {
			if (insure_fee > 0) {
				document.getElementById("unc_insure_cell").style.display = '';
				setValue(document.getElementById("unc_insure_fee_cell"), getFormatedPrice(insure_fee));
			} else {
				document.getElementById("unc_insure_cell").style.display = "none";
				setValue(document.getElementById("unc_insure_fee_cell"), '');
			}
		}
	}
}
...
function showCatalog(obj) {
	...
	var div = document.getElementById('unc_CATALOG');
...
function orderQueryResponse(result) {
	...
	if (result.error == 0) {
		var div = document.getElementById('unc_ORDER_QUERY');
 ...
 * 添加礼包到购物车
 */
/* 代码修改_start By www.m775.com 这里增加了一个变量 */
...
function opencartDiv(price, name, pic, goods_brief, goods_id, total, number) {
	...
	// 生成层内内容
	html = '<div class=cardivfloat><span class=cartdivfloattitle>商品已成功添加到购物车！</span><a href=\'javascript:cancel_div()\' style="float:right;padding:0 26px 0 0;background:url(themes/m775_shop/images/ico_closebig1.gif) right center no-repeat;cursor:pointer;color:#ffffff;font-size:12px;" >关闭</a></div><div class="cartpopDiv"><div class="toptitle"><a href="goods.php?id=' + goods_id + '" class="pic"><img src=' + pic + ' width="98" height="98" style="border:#dddddd 1px solid; display:block;"/></a><p><font style="font-weight:bold">' + name + '</font>  <br>' + goods_brief + '<br>购买价格：<font style="color:#cc0000">' + price + '</font><br></p></div>';
	...
	html += "<div class=cartbntfloat ><a href='flow.php'><img src='themes/m775_shop/images/jsico1.gif'></a><a href=\'javascript:cancel_div()\'><img src='themes/m775_shop/images/goon_ico1.gif'></a></div>";
	html += '</div></div>';
